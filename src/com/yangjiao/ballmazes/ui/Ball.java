package com.yangjiao.ballmazes.ui;

import java.util.Timer;
import java.util.TimerTask;

import com.yangjiao.ballmazes.core.Direction;
import com.yangjiao.ballmazes.core.GameEngine;
import com.yangjiao.ballmazes.core.Messages;
...
public class Ball {
	...
	private volatile boolean mIsRolling = false;
	private volatile Direction mRollDirection = Direction.NONE;
	
	public Ball(GameEngine engine, Puzzle map, int init_x, int init_y) {
		mEngine = engine;
		mMap = map; 
		mX = init_x;
		mY = init_y;
	...
	public synchronized boolean roll(Direction dir) {
		...
		if (mXTarget == Math.round(mX) && mYTarget == Math.round(mY)) return false;
		//Log.e("ernest", "mXTarget="+mXTarget+",mYTarget="+mYTarget+",mX="+Math.round(mX)+",mY="+Math.round(mY));
		mIsRolling = true;
		mRollDirection = dir;
